<html> 
<head> 
<title> Kanish group | Carving Face Engraving </title>
...
<body>
	...
	<h1> Face Engraving </h1>
	
	<?php
		include("connection.php");
		$db = mysql_select_db("kanishGroupDb",$connection);

		$table = mysql_query("SELECT id, faceengraving FROM carving ORDER BY id ASC");
		
			echo "<div class='imgWrapper'>";
			echo "<ul>";
		while($row=mysql_fetch_array($table)){
			$id = $row['id'];
			$img = $row['faceengraving'];
			if(!empty($img)){
			echo "<li> <img src='../images/carving/$img' />  
			
				<form action='remove_carvingfaceEngraving.php?id=$id' name='remove' method='post' onsubmit='return(removeFun())'> <input type='submit' class='removeBtn' name='remove' value='REMOVE' /> </form> </li> 
			";
			}
	...
	?>
		...
		<?php
			...
			if(isset($_POST['submit'])){		
				...
				if (isset($fileName)){
					$upload = mysql_query("INSERT INTO carving (faceengraving)VALUE('$fileName')");				
