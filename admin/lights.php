<html> 
<head> 
<title> Kanish group | Interior &amp; Exterior Lights </title>
...
<body>
	...
	<h1> Interior &amp; Exterior Lights </h1> 
	
	<?php
		include("connection.php");
		$db = mysql_select_db("kanishGroupDb",$connection);
		$table = mysql_query("SELECT *FROM products WHERE interiorlights='interiorlights' ORDER BY id ASC"); 
		...
		while($row=mysql_fetch_array($table)){
			...
			echo "<li> <img src='../images/products/$img' />  
				...
				<form action='removeLights.php?id=$id' name='remove' method='post' onsubmit='return(removeFun())'> <input type='submit' class='removeBtn' name='remove' value='REMOVE' /> </form> </li> 
			";
	...
	?>
		...
		<?php
			...
			if(isset($_POST['submit'])){		
				...
				if (isset($fileName)){
					$upload = mysql_query("INSERT INTO products (interiorlights, image)VALUE('interiorlights','$fileName')");				
