...
<body>
	...
	<?php
		...
		$table = mysql_query("SELECT *FROM hylamsheet ORDER BY id ASC");
		...
		while($row=mysql_fetch_array($table)){
			$id = $row['id'];
			$img = $row['image'];
			echo "<li> <img src='../images/hylamsheet/$img' />  
			
				<form action='removeHylamsheet.php?id=$id' name='remove' method='post' onsubmit='return(removeFun())'> <input type='submit' class='removeBtn' name='remove' value='REMOVE' /> </form> </li> 
			";
	...
	?>
		...
		<?php
			...
			if(isset($_POST['submit'])){		
				...
				$targetFile = "../images/hylamsheet/" . $fileName;
				if (isset($fileName)){
					$upload = mysql_query("INSERT INTO hylamsheet (image)VALUE('$fileName')");				
