<html> 
<head> 
<title> Kanish group | Carving Face Embossing </title>
...
<body>
	...
	<h1> Face embossing </h1>
	
	<?php
		include("connection.php");
		$db = mysql_select_db("kanishGroupDb",$connection);

		$table = mysql_query("SELECT id, faceembossing FROM carving ORDER BY id ASC");
		
			echo "<div class='imgWrapper'>";
			echo "<ul>";
		while($row=mysql_fetch_array($table)){
			$id = $row['id'];
			$img = $row['faceembossing'];
			if(!empty($img)){
			echo "<li> <img src='../images/carving/$img' />  
			
				<form action='remove_carvingfaceEmbossing.php?id=$id' name='remove' method='post' onsubmit='return(removeFun())'> <input type='submit' class='removeBtn' name='remove' value='REMOVE' /> </form> </li> 
			";
			}
	...
	?>
		...
		<?php
			...
			if(isset($_POST['submit'])){		
				...
				if (isset($fileName)){
					$upload = mysql_query("INSERT INTO carving (faceembossing)VALUE('$fileName')");				
