...
<body>
	...
	$findImg = mysql_query("SELECT *FROM printing WHERE id='$getId'");
			...
			while($row=mysql_fetch_array($findImg)){
				...
				if(file_exists("../images/glass/$imgName")){
					$unlinked = unlink("../images/glass/$imgName");
					if($unlinked){
						$removed = mysql_query("DELETE FROM printing WHERE id='$getId'");
						if($removed){
							echo "<div class='imgWrapper'> IMAGE REMOVED <br /> <a href='glassdoor.php'> BACK </a> </div>";
						}else{
							echo mysql_error();
						}
					}
