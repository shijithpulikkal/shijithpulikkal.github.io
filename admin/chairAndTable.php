<html> 
<head> 
<title> Kanish group | Chair &amp; Tableset </title>
...
<body>
	...
	<h1> Chair &amp; Table Set  </h1>
	
	<?php
		include("connection.php");
		$db = mysql_select_db("kanishGroupDb",$connection);
		$table = mysql_query("SELECT *FROM products WHERE chairtable='chairtable' ORDER BY id ASC");
		...
		while($row=mysql_fetch_array($table)){
			...
			echo "<li> <img src='../images/products/$img' />  
				...
				<form action='removeChairAndTable.php?id=$id' name='remove' method='post' onsubmit='return(removeFun())'> <input type='submit' class='removeBtn' name='remove' value='REMOVE' /> </form> </li> 
			";
	...
	?>
		...
		<?php
			...
			if(isset($_POST['submit'])){		
				...
				if (isset($fileName)){
					$upload = mysql_query("INSERT INTO products (chairtable, image)VALUE('chairtable','$fileName')");				
