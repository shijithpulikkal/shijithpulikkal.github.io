...
<body>
	...
	<?php
		...
		$table = mysql_query("SELECT *FROM forex ORDER BY id ASC");
		...
		while($row=mysql_fetch_array($table)){
			$id = $row['id'];
			$img = $row['image'];
			echo "<li> <img src='../images/forex/$img' />  
			
				<form action='removeForex.php?id=$id' name='remove' method='post' onsubmit='return(removeFun())'> <input type='submit' class='removeBtn' name='remove' value='REMOVE' /> </form> </li> 
			";
	...
	?>
		...
		<?php
			...
			if(isset($_POST['submit'])){		
				...
				$targetFile = "../images/forex/" . $fileName;
				if (isset($fileName)){
					$upload = mysql_query("INSERT INTO forex (image)VALUE('$fileName')");				
