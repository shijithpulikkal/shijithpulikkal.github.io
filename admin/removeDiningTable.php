	...
	<?php
		...
		$findImg = mysql_query("SELECT *FROM interiorworks WHERE id='$getId'");
			...
			while($row=mysql_fetch_array($findImg)){
				...
				if(file_exists("../images/interiorworks/diningTable/$imgName")){
					$unlinked = unlink("../images/interiorworks/diningTable/$imgName");
					if($unlinked){
						$removed = mysql_query("DELETE FROM interiorworks WHERE id='$getId'");
						if($removed){
							echo "<div class='imgWrapper'> IMAGE REMOVED <br /> <a href='diningTable.php'> BACK </a> </div>";
						}else{
							echo mysql_error();
						}
					}
